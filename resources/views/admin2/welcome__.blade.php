...
@section('center_content')
    ...
    @component('component.card', ['title' => 'Добро пожаловать в CASVA', 'bodyClass' => 'card-body-no-padding'])
            ...
            <div class="col-sm-3">
                <div class="left-bar d-flex flex-column align-items-center border rounded-lg p-3 pb-3" style="background-color: #002C47;">
                    ...
                    <div class="text-center mb-3">
                        ...
                        <div class="text-green">{{ \App\Domain\TruckBookings\Models\TruckBooking::where('company_id', $company->id)->where('status', 'done')->sum('price') }} сум
                    ...
                    <div class="text-center mb-3">
                        ...
                        <div class="text-green">{{ \App\Domain\TruckBookings\Models\TruckBooking::where('company_id', $company->id)->where('status', 'done')->sum('commission') }} сум
                ...
                <div class="border rounded-lg px-3 py-1">
                    <div class="text-primary d-flex align-items-center justify-content-between" style="font-size: 18px;">Завершенные
                        <span class="text-green" style="font-size: 35px;">{{ \App\Domain\TruckBookings\Models\TruckBooking::where('company_id', $company->id)->where('status', 'done')->count('commission') }}</span>
                ...
                <div class="border rounded-lg mt-3  px-3 py-1">
                    <div class="text-primary d-flex align-items-center justify-content-between" style="font-size: 18px;">Выполняемые
                        <span class="text-warning" style="font-size: 35px;">{{ \App\Domain\TruckBookings\Models\TruckBooking::where('company_id', $company->id)->whereIn('status', ['waiting', 'accepted', 'arrived', 'processing', 'pause'])->count('commission') }}</span>
