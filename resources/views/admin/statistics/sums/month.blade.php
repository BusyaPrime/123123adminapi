    ...
    @component('component.card', ['title' => 'Оборот и отчисления', 'bodyClass' => 'card-body-no-padding'])
        ...
        @slot('buttons')
            ...
            <div class="row col-sm-6 justify-content-end">
                <a href="{{ route('admin.statistics.sums.year') }}" class="btn btn-default  mr-3">Год</a>
...
@push('scripts')
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                labels: [
                    @for($i = 1; $i <= $sums_count['days_in_month']; $i++)
                        '{{ $i }}',
                    @endfor
                ],
                datasets: [{
                ...
                },
                    {
                        label: 'Отчисления',
                        data: [
                            @for($i = 1; $i <= $commission_count['days_in_month']; $i++)
