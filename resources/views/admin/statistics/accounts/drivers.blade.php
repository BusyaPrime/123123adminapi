    ...
    @component('component.card', ['title' => 'Аналитика по аккаунтам', 'bodyClass' => 'card-body-no-padding'])
        ...
        @slot('buttons')
            <div class="row col-sm-6 justify-content-end">
                <form id="statistics_form" class="col-12 row" action="{{ route('admin.statistics.accounts.drivers') }}">

                    <div class="col-9 text-right">
                        <a href="{{ route('admin.statistics.accounts.drivers.export', ['preset' => $preset]) }}" class="btn btn-default mr-3">
                            <span class="d-none d-sm-inline-block">Экспорт</span> <i class="icmn-plus"><!-- --></i>
                        </a>
                        <a  class="btn btn-primary active   mr-3">Водители</a>
                        <a href="{{ route('admin.statistics.accounts.users') }}" class="btn btn-default mr-3">Пользователи</a>
                    </div>
                    <div class="col-3">
                ...
                <table class="table">
                    ...
                    @foreach($users as $user)
                        <tr class="table__tr mt-2 mb-2 " >
                            <td class="table__td">{{ $user->id ?? $user->car->id }}</td>
                            ...
                            <td class="table__td">
                                {{ $user->car->created_at->format('d.m.Y') }}
                            </td>
                            <td class="table__td">
                                {{ \App\Domain\TruckBookings\Models\TruckBooking::where('driver_id', $user->id)->count() ?? 0}}
                            </td>
                            <td class="table__td">
