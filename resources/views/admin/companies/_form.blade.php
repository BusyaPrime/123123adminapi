...
<input type="hidden"
       name="companyRole"
       value="{{ old('companyRole', $company->role ?? \App\Domain\Companies\Models\Company::ROLE_COMPANY) }}">
...
<div class="row">
    ...
    @if($company && $company->user->is_external)
    <div class="col-sm-3">
        <div class="form-group  {!! $errors->first('bank', 'has-danger')!!}">
            <label class=" text-md-right text-secondary">Лимит задолженности</label>
            <div class="">
            <select name="priority_id" id="priority_id" class="form-control input-sm">
                <option value='0'>Нет</option>
            @foreach($company_priorities as $item)
                <option value="{{ $item->id }}" {{ old('priority_id', $company->priority_id ?? '') == $item->id ? 'selected': '' }}>{{ $item->name }} - {{ number_format($item->quantity, 0, '', ' ') }} сум</option>
            @endforeach
        </select>
                <!-- <input type="number" name="bank" class="form-control input-sm bank" id="bank" value="{{ old('bank', $company->bank ?? '') }}" > -->
                {!! $errors->first('bank', '<small class="form-control-feedback">:message</small>') !!}
            </div>
        </div>
    </div>
...
<div class="row">
	...
	@if($company && $company->user->is_external == 0)
    <div class="col-md-3">
        <div class="form-group  ">
            <label class=" text-md-right text-secondary" for="commission_rate_id">Комиссия</label>
            <div class="">
                <select name="commission_rate_id" id="commission_rate_id" class="form-control" >
                    @foreach(\App\Domain\CommissionRate\Models\CommissionRate::get() as $rate)
                        <option value="{{ $rate->id }}" {{ old('commission_rate_id', $company->commission_rate_id ?? 0) == $rate->id ?'selected':'' }}>{{ $rate->title }} ({{$rate->commission ?? 0}}%)</option>
                    @endforeach
                </select>
            </div>
        </div>
	...
	<div class="col-md-3">
        <div class="form-group  {!! $errors->first('phonenumber', 'has-danger')!!}">
            <label class=" text-md-right text-secondary" for="phonenumber">Phone Number*</label>
            <div class="">
                <input type="text" name="phonenumber" class="form-control input-sm" id="phonenumber" value="{{ old('phonenumber', $user->profile->phone_number ?? '') }}" required>
