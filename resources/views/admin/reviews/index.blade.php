        ...
        @slot('filters')
            <section class="content" id="filter_block" style="{{ $filters['id'] != '' || $filters['user_phone'] != '' || $filters['driver_phone'] != '' ?'':'display: none' }}">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <form method="get" action="{{ route('admin.reviews.index') }}" id="filter_block_form">
                                <div class="border rounded p-3">
                                    ...
                                    <div class="row align-items-center justify-content-end">
                                        <div class="col-sm-6 d-flex align-items-center justify-content-end">
                                            <a href="{{ route('admin.reviews.index') }}" class="btn btn-danger mr-2" style="{{ $filters['id'] != '' || $filters['user_phone'] != '' || $filters['driver_phone'] != '' }};">Сбросить фильтры</a>
                ...
                <div class="row border rounded justify-content-center mb-4" style="border-color: #858585!important;">
                    <div class="col-sm-12 d-flex justify-content-between border-bottom" style="border-color: #C4C4C4!important;">
                        ...
                        <div class="p-2 text-sm text-center col-sm-3 border-right" style="border-color: #C4C4C4!important;">
                            @php
                            $regionFrom = \App\Domain\Regions\Models\Region::find($review->region_from_id);
                            $regionTo = \App\Domain\Regions\Models\Region::find($review->region_to_id);
                            @endphp
                            {{ $regionFrom->title ?? 'Регион не определен' }} &#10141 {{ $regionTo->title ?? 'Регион не определен' }}</div>
                        ...
                        <div class="p-2 text-sm text-center col-sm" >
                            <a href="{{ route('admin.reviews.delete', ['booking' => $review, 'type' => 'review']) }}"
                            onclick="if (!confirm('@lang('admin.destroy_confirm')')) {
                                return false;
                                }">
                                <i class="fas fa-trash-alt mt-1 text-muted" style="font-size: 24px"></i>
                            </a>
                        </div>
                    </div>
                    <div class="col-sm-12 row">
                        <div class="col-sm-6 p-3 border-right" style="border-color: #C4C4C4!important;">
                            <div class="col-sm-12 d-flex border rounded justify-content-between p-2" style="border-color: #C4C4C4!important;">
                                ...
                                <div class="mr-3 d-flex flex-column justify-content-between">
                                    @if($review->user)
                                        <a href="{{ route('admin.users.show', $review->user->user_id) }}" >
                                            <i class="fas fa-eye  text-muted" ></i>
                                        </a>
                                    @endif
                                    @if($review->rating > 0)
                                        <a href="{{ route('admin.reviews.delete', ['booking' => $review, 'type' => 'user']) }}" class="mt-auto"
                                           onclick="if (!confirm('@lang('admin.destroy_confirm')')) {
                                               return false;
                                               }">
                        ...
                        <div class="col-sm-6 p-3">
                            <div class="col-sm-12 d-flex border rounded justify-content-between p-2" style="border-color: #C4C4C4!important;">
                                <div class="d-flex justify-content-between ">
                                    ...
                                    <div class="text-sm mr-3">
                                        <p class="mb-2">{!! $review->driver_id ? (($review->driver->surname ?? '').' '.($review->driver->name ?? '').' '.($review->driver->middle_name ?? '')) : '' !!}
                                            ({!! $review->company ? '<a href="'.(route('admin.companies.show', $review->company)).'">'.$review->company->title.'</a>': 'Самозанятый' !!})
                                        </p>
                                ...
                                <div class="mr-3 d-flex flex-column justify-content-between">
                                    @if($review->driver)
                                        @if(isset ($review->driver->user) && isset($review->driver->user->car))
                                            <a href="{{ route('admin.cars.show', $review->driver->user->car->id ?? 0) }}" class="">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                        @else
                                            <a href="{{ route('admin.users.show', $review->driver->user_id ?? 0) }}" class="">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                        @endif
                                    @endif
                                        @if($review->driver_rating > 0)
                                            <a href="{{ route('admin.reviews.delete', ['booking' => $review, 'type' => 'driver']) }}" class="mt-auto"
                                               onclick="if (!confirm('@lang('admin.destroy_confirm')')) {
                                                   return false;
                                                   }">
                                                <i class="fas fa-trash-alt  text-muted" ></i>
