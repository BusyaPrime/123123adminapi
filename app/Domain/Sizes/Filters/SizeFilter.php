...
namespace App\Domain\Sizes\Filters;
...
class SizeFilter extends Filter
{
    ...
    protected $translationTable = 'size_translations';
    ...
    protected function init()
    {
        ...
        $this->addJoin($this->translationTable, function () {
            $this->builder->leftJoin($this->translationTable, function ($join) {
                 ...
                 */
                $join->on($this->table . '.id', $this->translationTable . '.size_id')->where('locale', \App::getLocale());
            })->select($this->table . '.*');
        });
    }
