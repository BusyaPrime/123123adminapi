...
namespace App\Domain\LoadTypes\Filters;


use App\Services\FilterService\Filter;
use Illuminate\Http\Request;

class LoadTypeFilter extends Filter
{
    ...
    protected $translationTable = 'load_type_translations';
    ...
    protected function init()
    {
        ...
        $this->addJoin($this->translationTable, function () {
            $this->builder->leftJoin($this->translationTable, function ($join) {
                 ...
                 */
                $join->on($this->table . '.id', $this->translationTable . '.load_type_id')->where('locale', \App::getLocale());
            })->select($this->table . '.*');
        });
    }
