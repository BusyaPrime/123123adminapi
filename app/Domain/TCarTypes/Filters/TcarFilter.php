...
namespace App\Domain\TCarTypes\Filters;
...
class TcarFilter extends Filter
{
    ...
    protected $translationTable = 'tcar_type_translations';
    ...
    protected function init()
    {
        ...
        $this->addJoin($this->translationTable, function () {
            $this->builder->leftJoin($this->translationTable, function ($join) {
                 ...
                 */
                $join->on($this->table . '.id', $this->translationTable . '.tcar_type_id')->where('locale', \App::getLocale());
            })->select($this->table . '.*');
        });
    }
