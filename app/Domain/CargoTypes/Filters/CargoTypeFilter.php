...
namespace App\Domain\CargoTypes\Filters;


use App\Services\FilterService\Filter;
use Illuminate\Http\Request;

class CargoTypeFilter extends Filter
{
    ...
    protected $translationTable = 'cargo_type_translations';
    ...
    protected function init()
    {
        ...
        $this->addJoin($this->translationTable, function () {
            $this->builder->leftJoin($this->translationTable, function ($join) {
                 ...
                 */
                $join->on($this->table . '.id', $this->translationTable . '.cargo_type_id')->where('locale', \App::getLocale());
            })->select($this->table . '.*');
        });
    }
