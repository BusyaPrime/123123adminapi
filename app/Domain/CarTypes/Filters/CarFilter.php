...
namespace App\Domain\CarTypes\Filters;
...
class CarFilter extends Filter
{
    ...
    protected $translationTable = 'car_type_translations';
    ...
    protected function init()
    {
        ...
        $this->addJoin($this->translationTable, function () {
            $this->builder->leftJoin($this->translationTable, function ($join) {
                 ...
                 */
                $join->on($this->table . '.id', $this->translationTable . '.car_type_id')->where('locale', \App::getLocale());
            })->select($this->table . '.*');
        });
    }
